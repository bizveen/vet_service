  ...
  if (images != null) {
    images.forEach((element) {
      FirebaseStorageMethods().uploadImageToStorage(
          ...
          file: element,
          folderPath: 'pets/${petId}/timeLine',
          title: 'Timeline');
    });
  }
}
...
DateTime? stringToDate(String? dateTime) {
  ...
  if(!(dateTime.split('/')[0].isNumber)){return null;}
  if (dateTime.split(' ').length > 1) {
    return DateTime(
      ...
      int.parse(dateTime.split(':')[0].substring(
          dateTime.split(':')[0].length - 2, dateTime.split(':')[0].length)),
      int.parse(dateTime.split(':').length > 1 ? dateTime.split(':')[1] : '0'),
      int.parse(dateTime.split(':').length > 2 ? dateTime.split(':')[2] : '0'),
    );
  } else {
    print(dateTime);
    return DateTime(
